...
def draw(): # Pygame Zero draw function
    ...
    if gameStatus == 0: # display the title page
        drawCentreText("VIRUS INVADERS\n\n\nIngresar tu Nombre y luego\npressionar Enter para empezar\n(Muevete con las flechas, dispara con la barra espaciadora)")
    ...
    if gameStatus == 1: # playing the game
        ...
        if player.status >= 30:
            if player.lives > 0:
                drawCentreText("¡TE DIERON!\nPresionar ENTER para regenerarte")
            else:
                drawCentreText("¡EL JUEGO TERMINO!\nPresionar ENTER para continuar")
        if len(aliens) == 0 :
            drawCentreText("¡NIVEL COMPLETADO!\nPresionar ENTER para pasar al siguiente nivel")
    if gameStatus == 2: # game over show the leaderboard
        drawHighScore()

def drawCentreText(t):
    screen.draw.text(t , center=(400, 300), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=40)
...
def drawHighScore():
    global highScore
    y = 0
    screen.draw.text("PUNTAJES ALTOS", midtop=(400, 30), owidth=0.5, ocolor=(255,255,255), color=(0,64,255) , fontsize=60)
    for line in highScore:
        if y < 400:
            screen.draw.text(line, midtop=(400, 100+y), owidth=0.5, ocolor=(0,0,255), color=(255,255,0) , fontsize=50)
            y += 50
    screen.draw.text("Presionar ESC para jugar nuevamente" , center=(400, 550), owidth=0.5, ocolor=(255,255,255), color=(255,64,0) , fontsize=60)
...
def initAliens():
    ...
    for a in range(24):
        if int(a/6) == 0:
            aliens.append(Actor("1a", (210+(a % 6)*80,100+(int(a/6)*64))))
        elif int(a/6) == 1:
            aliens.append(Actor("2a", (210+(a % 6)*80,100+(int(a/6)*64))))
        elif int(a/6) == 2:
            aliens.append(Actor("3a", (210+(a % 6)*80,100+(int(a/6)*64))))
        elif int(a/6) == 3:
            aliens.append(Actor("4a", (210+(a % 6)*80,100+(int(a/6)*64))))
